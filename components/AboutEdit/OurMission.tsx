...
const OurMission = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = isEditing ? `/api/OurMission/${editingId}` : '/api/OurMission';
  ...
  const handleDelete = async (id: number) => {
    ...
    const res = await fetch(`/api/OurMission/${id}`, { method: 'DELETE' });
    if (res.ok) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow rounded">
      ...
      <div className="mt-8">
        <h3 className="text-lg font-semibold mb-2">Images List</h3>
