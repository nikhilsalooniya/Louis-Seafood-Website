...
const FishPage = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Fish product added successfully');
        // After successful product addition, fetch products again
        const updatedProducts = await fetch('/api/fish');
  ...
  return (
    <div className="space-y-6  p-6 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-black">Add Fish Product</h2>
        ...
        <ul className="space-y-4 mt-4">
          {products.map((product) => (
            <li key={product.id} className="flex justify-between items-center">
              ...
              <button
                onClick={async () => {
                  await fetch(`/api/fish/${product.id}`, { method: 'DELETE' });
