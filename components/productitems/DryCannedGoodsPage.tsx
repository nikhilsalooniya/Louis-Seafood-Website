...
const DryCannedGoodsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const response = await fetch('/api/dry-canned-goods');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Dry & Canned Goods product added successfully');
        // After successful product addition, fetch products again
        const updatedProducts = await fetch('/api/dry-canned-goods');
  ...
  return (
    <div className="space-y-6 p-6 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-black">Add Dry & Canned Goods Product</h2>
        ...
        <ul className="space-y-4 mt-4">
          {products.map((product) => (
            <li key={product.id} className="flex justify-between items-center">
              ...
              <button
                onClick={async () => {
                  await fetch(`/api/dry-canned-goods/${product.id}`, { method: 'DELETE' });
...
export default DryCannedGoodsPage;
