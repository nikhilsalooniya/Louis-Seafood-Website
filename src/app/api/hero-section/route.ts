...
export async function POST(req: NextRequest) {
  try {
    ...
    const videoUrl = uploadResult.secure_url; // Cloudinary video URL

    await pool.query(
      'INSERT INTO hero_section (heading, subheading, video_url) VALUES ($1, $2, $3)',
...
export async function GET() {
  try {
    const result = await pool.query('SELECT * FROM hero_section ORDER BY id DESC LIMIT 1');
