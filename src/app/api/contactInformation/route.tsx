...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Insert into DB
    await pool.query(
      'INSERT INTO contactinfo (address, number, email) VALUES ($1, $2, $3)',
      [address, number, email]
    );

    return NextResponse.json({ message: 'Contact section saved successfully!' }, { status: 201 });
  } catch (err) {
    console.log(err)
...
export async function GET() {
  try {
    const result = await pool.query('SELECT * FROM contactinfo ORDER BY id DESC LIMIT 1');
