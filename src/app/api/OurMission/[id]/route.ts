...
export async function PUT(req: NextRequest, context: any) {
  ...
  try {
    ...
    const existingResult = await pool.query('SELECT * FROM ourmission WHERE id = $1', [offerId]);
    ...
    const updateResult = await pool.query(
      'UPDATE ourmission SET title = $1, image_url = $2 WHERE id = $3 RETURNING *',
      [title, updatedImageUrl, offerId]
    );
...
export async function DELETE(req: NextRequest, context: any) {
  ...
  try {
    ...
    const deleteResult = await pool.query('DELETE FROM ourmission WHERE id = $1 RETURNING *', [offerId]);
